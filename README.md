...
- `codec_pipeline.chunk_cache_budget_bytes`, `codec_pipeline.chunk_cache_min_items`: tune the per-read cache that `integer_array_indexing` uses when several runs land in one *inner* chunk, so that inner chunk is decoded once rather than once per run.
  - Default `268435456` (256 MiB) and `4`. Extracting any part of an inner chunk decodes all of it, so the cache is worth it in proportion to how many runs share one. A chunk is cached only when its runs average at least `chunk_cache_min_items` per inner chunk and the inner chunks they touch fit the budget; a selection spread one run per inner chunk gains nothing and is left uncached. Only the touched inner chunks are held, not the whole shard. Set the budget to `0` to disable the cache and keep the rest of `integer_array_indexing`.
