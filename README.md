...
Reads of a sharded array opened read-only also **remember each shard's decoded index** for the life of the array, so a shard's index costs one round trip rather than one per read that touches it. This is automatic and has no option. It assumes nothing else is rewriting the array while it is open: the same caveat as `file_handle_cache_size` below, for the same reason, and it applies only to arrays opened with `mode="r"`.
