...
- `codec_pipeline.shard_index_cache_size`: the number of shard indexes kept in a least-recently-used cache. If nonzero, a partial read of a shard reuses the index decoded by an earlier read instead of fetching and decoding it again, which removes one read per shard per call. Like the file handle cache, this matters most on network filesystems and for repeated small reads of the same shards, such as random-access sampling.
  ...
  - Read-only by design: any write through this pipeline empties the cache entirely rather than tracking which entry it invalidated, because a write moves where a shard's inner chunks live and being wrong about that is silently wrong data. Modification from anywhere else is still not seen — the same caveat as `file_handle_cache_size` above — so only enable this while nothing else is modifying the array.
- `codec_pipeline.integer_array_indexing`: serve reads with a sorted integer-array index (`z[[3, 7, 8, 9], :]`, `z[:, cols]`, 1-D `z.vindex[idx]`) through `zarrs` instead of falling back to the `zarr-python` pipeline. A chunk read is described to `zarrs` as a rectangular subset, and such a selection is not one rectangle but a stack of them, so the pipeline emits one per run of consecutive indices.
  - Defaults to `False`, because whether it helps depends entirely on how dense the selection is — measure before turning it on. Each run is a separate decode, so runs landing in the same inner chunk of a shard decode that inner chunk once each, where the `zarr-python` pipeline decodes it once in total and additionally coalesces nearby inner-chunk byte ranges into single reads. The cost is therefore set by how many selected rows share an inner chunk, which is what `plan_reads` below removes. Measured on a local SSD with a release build, sampling rows of a `(40000, 1000)` array with `chunks=(100, 1000)` (400 inner chunks):

    | sampled rows | rows per touched inner chunk | alone | + `plan_reads` |
    | --- | --- | --- | --- |
    | 32 | 1.00 | 0.37x | 0.33x |
    | 128 | 1.24 | 0.48x | 0.35x |
    | 512 | 1.75 | 0.89x | 0.51x |
    | 2048 | 5.13 | 2.86x | 0.82x |
    | 8192 | 20.48 | 9.19x | 1.58x |

    Ratios are against the `zarr-python` fallback, so below `1.00x` is faster. On its own this option wins outright while each selected row lands in its own inner chunk and loses as the duplication grows linearly in the number of runs; `plan_reads` below removes that duplication, and is faster than the fallback at every density measured except the densest.
  - Reads only. Writes still fall back, because one chunk item per run would make several read-modify-writes of the same chunk race each other.
  - Applies only when the indices are sorted and strictly increasing, and when a single axis is an integer array. Unsorted or repeated indices, and two or more integer-array axes, still fall back: `zarr-python` reorders the output for those, so a run's position in the selection is not its position in the output.
- `codec_pipeline.plan_reads`: read the shard index directly and issue the byte ranges of exactly the inner chunks the selection needs, instead of asking the sharding partial decoder for one region at a time. Each needed inner chunk is fetched and decoded once regardless of how many requested boxes fall inside it.
  - Defaults to `False`. Needs `integer_array_indexing` to be useful, and applies only to a chunk whose codec chain is exactly the sharding codec — an array-to-array or bytes-to-bytes codec wrapping it means the shard bytes are not laid out as the index describes, so the array falls through to the normal path. Unsharded arrays fall through too.
  - Uses `shard_index_cache_size` for the parsed indexes.
  - Units from every shard are issued into one pool, so reads of one shard overlap each other's as well as other shards'. Where inner chunks must be fetched whole, fetching and decoding run on separate pools: a landed read hands one decode task per inner chunk straight to the decode pool rather than waiting for its siblings. A run spanning two inner chunks becomes two units, so no unit ever waits on two reads — which is what lets the budget below go as low as one read without stalling.
  - Sub-chunk reads are preserved: with no compressor an element's position in an inner chunk is arithmetic, so only the bytes asked for are read. A compressor makes the inner chunk atomic, and then it is fetched and decoded once however many requested regions fall inside it.
  - Measured on a local SSD with a release build, sampling rows of a CSR `data` array (31.2M elements, `chunks=(65536,)`, 475 inner chunks) — the shape of a batched sparse read, as `AnnData` stores one:

    | sampled rows | `plan_reads` | without it | fallback |
    | --- | --- | --- | --- |
    | 32 | **3.1 ms** | 3.7 ms | 11.7 ms |
    | 256 | **17.0 ms** | 21.7 ms | 43.2 ms |
    | 2048 | **56.0 ms** | 159.1 ms | 86.6 ms |
- `codec_pipeline.plan_reads_fetch_threads`: threads issuing reads for `plan_reads`. These block on IO, so they are a pool of their own rather than sharing the decode pool — a fetch waiting on the byte budget must not be able to occupy the thread that would release it.
  - Defaults to `0`, meaning the size of the `rayon` pool. Raise it for storage where per-request latency dominates and threads spend their time waiting. The pool is process-wide and sized on first use, so reading the several arrays of a sparse matrix does not create one pool per array.
- `codec_pipeline.plan_reads_fetch_byte_budget`: ceiling on bytes that have been fetched but not yet decoded. Without one, a read plan covering an array would fetch all of it before the first decode finished.
  - Defaults to `0`, meaning 256 MiB. A read larger than the whole budget is admitted on its own rather than never, so a small budget throttles rather than deadlocks.
- `codec_pipeline.plan_reads_merge_gap_bytes`: with `plan_reads`, merge two inner-chunk reads into one request when the gap between them is at most this many bytes.
  - Defaults to `0`, which merges only reads that touch — one chunk ending exactly where the next begins — so not a byte outside the request is ever read. That still collapses dense selections into single large reads, because a shard written in one pass lays its inner chunks down contiguously in index order.
  - Raising it reads unrequested bytes to save requests. That is a bandwidth-for-latency trade that depends on the storage, which is why it is a knob rather than a default; `zarr-python` makes the same trade at a fixed 1 MiB. A merged read never spans more than one shard.
