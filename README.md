...
- `codec_pipeline.integer_array_indexing`: serve reads with a non-decreasing integer-array index (`z[[3, 7, 8, 9], :]`) through `zarrs` instead of falling back. Each run of consecutive indices becomes one rectangular chunk read.
  - Defaults to `False`. Runs decode separately, so several inside one inner chunk decode it once each where `zarr-python` decodes it once -- roughly, the speedup is `zarrs`' decode advantage divided by the average runs per inner chunk. Enabling it is faster with long runs however many rows, and slower once enough runs share one inner chunk. Measure before enabling.
