...
- `codec_pipeline.read_workers` / `codec_pipeline.decode_workers`: the sizes of the two thread pools a read runs on, one fetching byte ranges from the store and one decoding chunks.
  ...
  - `read_workers` defaults to the number of CPUs this process may run on, but never fewer than 64, since reads in flight rather than cores are what a scattered selection is short of. `decode_workers` defaults to every CPU up to 16 and half of them beyond that. On spinning disks, raise `read_workers` well past this: several hundred readers kept paying off there.
