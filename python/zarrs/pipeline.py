...
# What sends a batch to zarr-python instead. `read` and `write` must use the same set: a member
# in one and not the other falls back on read and raises on write.
...
def get_codec_pipeline_impl(
    ...
    try:
        ...
        return CodecPipelineImpl(
            ...
            io_workers_max=config.get("codec_pipeline.io_workers_max", None),
            codec_workers_max=config.get("codec_pipeline.codec_workers_max", None),
            # zarr's own key for the codec pipeline's worker count, rather than one we invented.
            max_workers=config.get("codec_pipeline.max_workers", None),
...
class ZarrsCodecPipeline(CodecPipeline):
    ...
    async def read(
        ...
        try:
            ...
            # Read per call, not at open, so a `zarr.config.set` block around a read is honoured.
