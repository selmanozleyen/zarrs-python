...
def _chunk_unit_args(
    ...
    if isinstance(indices, slice):
        ...
        #
        # NOTE: a span read of a divided shard falls to the per-element path instead.
...
def _grid_unit_args(
    ...
    #
    # The all-absorbed case needs no branch: every axis is pinned, so the mesh is one point
    # and the ravel is that run's own start.
    cols = [
