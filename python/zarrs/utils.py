...
def _as_int64_batch_info(batch_info: BatchInfo) -> BatchInfo:
    ...
    def cast(sel: SelectorTuple) -> SelectorTuple:
        if isinstance(sel, np.ndarray):
            ...
            # only because uint64 arrives as float64, zarr having subtracted an intp offset.
            # Checked before casting: `astype` truncates 3.7 in silence, and comparing after
            # would cast 2**63 to i64::MAX, which compares equal.
    ...
    for dim_selection in selection:
        if is_integer(dim_selection):
            ...
            if len(dim_selection) == 0:
                # `dim_selection[0]` raises `IndexError`, which is not in
                # `FALLBACK_TO_ZARR_PYTHON` and would escape `read` rather than decline.
...
def chunk_info(
    ...
    """Describe a batch to Rust, one item per entry."""
