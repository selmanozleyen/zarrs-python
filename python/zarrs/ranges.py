...
def aread_ranges(
    ...
    *,
    ...
    """Read rows `starts[i] : starts[i] + lengths[i]` of axis 0, back to back, as a coroutine.
    ...
    if out is None:
        rows = (int(np.sum(lengths)), *a.metadata.shape[1:])
        out = np.empty(rows, dtype=a.metadata.dtype.to_native_dtype())
