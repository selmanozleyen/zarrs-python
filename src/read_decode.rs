...
pub(crate) struct ReadConfig {
    ...
    /// Decodes this call runs at once, out of the CPU pool.
...
/// arrival checks. Three separate entry points meant three lock acquisitions per read and, worse,
/// a check that could in principle run against different pools from the ones the defaults came
/// from.
pub(crate) fn config_for_call(
    ...
    let config = ReadConfig::from_call(py, io_workers, codec_workers, strict, &io, &codec)?;
...
mod tests {
    ...
    fn the_io_pool_is_wider_than_the_cpu_pool_and_both_report_what_they_built() {
        ...
        Python::attach(|py| {
            ...
            let config = config_for_call(py, None, None, true, limits).expect("resolvable");
