...
///
/// The raw path reads a row's exact bytes instead of the chunk around it, which is only
/// possible where the inner chunk is a plain byte tiling. That trades BYTES for REQUESTS, and
/// requests are the scarce resource: a 5.8 KB row costs nearly what a 366 KB chunk costs to
/// fetch, so many tiny reads lose to one large one however few bytes they move. The choice is
/// therefore PER ITEM -- take the raw path only where a chunk's wanted rows collapse to a
/// handful of reads.
///
/// Two is measured, not picked: raw pays at <= 2 reads per chunk and is flat to slightly
/// negative from 4 up. Zero disables the path, which is worth 75% on an uncompressed
/// scattered draw -- see `notes/deferred-wins.md`.
...
/// ready chunk needs.
///
/// ONE JOB PER READ, deliberately. Jobs sharing a shard used to be packed into groups of up
/// to eight and fetched with a single `get_partial_many` -- see the todo, which carries the
/// measurement. It is not in this commit because the store does not do what the packing
/// assumed: `get_partial_many` maps each range to its own `read_exact_at` and collects, so a
/// group was N SERIAL preads on one thread, sharing only the file-handle lookup, with no
/// decode starting until the last one finished. It is worth 5-7% on two fragmented dense
/// cells here and nothing elsewhere, and it costs a concept, a struct, two constants, two
/// counters and a knob. On higher-latency storage the handle sharing would be worth much
/// more; that is what makes it a todo rather than a rejection.
