...
pub(crate) struct ChunkItem {
    ...
    /// Linearised indices within the chunk, when the selection reached us as coordinates
    /// rather than as a rectangle. One item then covers a whole chunk's selection.
...
impl ChunkItem {
    ...
    fn new(
        ...
        // With coordinates the chunk subset is the bounding box, which is deliberately
        // larger than the selection, so the element counts do not have to agree.
        if coords.is_none()
