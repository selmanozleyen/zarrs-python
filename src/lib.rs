...
pub(crate) struct CodecPipelineImpl {
    ...
    /// The widths this array asked the pools for; see `read_decode::PoolLimits`.
    pub(crate) limits: read_decode::PoolLimits,
...
impl CodecPipelineImpl {
    ...
    fn retrieve_items_and_apply_index(
        ...
        if let Some(shard) = self.shard.as_ref() {
            ...
            let pools = read_decode::pools(py, self.limits)?;
            py.detach(|| {
                if chunk_descriptions.is_empty() {
                    // No items is only correct when there is also nothing to fill: the output is
                    // `np.empty`, so returning it would hand back uninitialised memory as data --
                    // the very thing `DisjointBytes::covered` refuses once a job exists.
                    return if output_len == 0 {
                        Ok(())
                    } else {
                        Err(PyRuntimeError::new_err(format!(
                            "no chunk items for {output_len} output bytes, which would be \
                             returned uninitialised"
                        )))
                    };
                }
                // Inner concurrency is 1: parallelism is one job per chunk, not threads inside
                // one. The jobs override this per chunk anyway; this is the base they start from.
                let codec_options = self.codec_options.clone().with_concurrent_target(1);
...
impl CodecPipelineImpl {
    ...
    fn new(
        ...
        io_workers_max: Option<usize>,
        codec_workers_max: Option<usize>,
        ...
        // Resolved here and only here: the widths this array will ask the pools for. Nothing is
        // BUILT yet -- the pools are built by the first read or write -- so opening an array that
        // is never touched costs no threads.
        let limits = read_decode::resolve_limits(io_workers_max, codec_workers_max, max_workers)?;
    ...
    /// no second Rust path.
    #[pyo3(signature = (chunk_items, value, io_workers=None, codec_workers=None, raw_max_reads_per_chunk=None, strict=false))]
    fn retrieve_chunk_items_and_apply_index(
        ...
        // Every width is a per-call decision, so none is a constructor argument. The one
        // width still not servable is above what a pool was built with, since it cannot grow.
        let config = read_decode::config_for_call(
            py,
            io_workers,
            codec_workers,
    ...
    #[pyo3(signature = (chunk_descriptions, value, write_empty_chunks, io_workers=None, codec_workers=None, strict=false))]
    fn store_chunks_with_indices(
        ...
        // The codec's own inner concurrency is 1 here, as it is on the read path: parallelism
        // comes from doing many chunks at once, not from threads inside one. Measured on writes
        // (`notes/write-path.md`): outer-only matches at zstd 0 and 9 and is 1.42x faster at 19,
        // where the encode dominates and the inner split was supposed to pay.
        let mut codec_options = self.codec_options.clone().with_concurrent_target(1);
        codec_options.set_store_empty_chunks(write_empty_chunks);

        // The same two knobs the read path uses, against the same two pools.
        let config = read_decode::config_for_call(
            py,
            io_workers,
            codec_workers,
            // A write never takes the raw path: it exists to avoid decoding, and a write encodes.
            None,
            strict,
            self.limits,
        )?;

        // Taken with the GIL held, as `pools` requires, and held across the `detach` below.
        let (_io_pool, encode_pool) = read_decode::pools(py, self.limits)?;
