...
pub struct CodecPipelineImpl {
    ...
    /// Whether zarr-python opened this store read-only. Carried because nothing else can
    /// enforce it: zarr-python's check lives in `StorePath.set`, which this pipeline
    /// bypasses, and `StoreConfig` builds a writable Rust store whatever the mode.
