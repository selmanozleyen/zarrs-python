...
impl CodecPipelineImpl {
    ...
    fn new(
        ...
        // Read off before `writable_store` is moved into the struct below.
        let read_only = writable_store.is_none();
        ...
        Ok(Self {
            ...
            // Upstream resolves read-only into the store itself and keeps no writable handle
            // for one, so its absence IS the read-only signal. This was a `store_is_read_only`
            // bool plumbed from `pipeline.py`; the information now comes from the same place
            // the write guard reads it.
            cache_shard_indexes: read_only,
        })
    }
    ...
    fn retrieve_chunk_items_and_apply_index(
        ...
        let config = read_decode::ReadConfig::new(read_worker_ceiling, decode_worker_ceiling);
