...
impl ByteBudget {
    ...
    fn admit(&self, bytes: u64) -> Credit<'_> {
...
impl CodecPipelineImpl {
    ...
    fn read_sub_chunk_units(
        ...
        let budget = ByteBudget::new(options.fetch_byte_budget);
    ...
    fn read_whole_units(
        ...
        let largest = whole_units
            ...
            .max()
            .unwrap_or(0);
        let budget = ByteBudget::new(options.fetch_byte_budget.max(largest));
        let failure: Mutex<Option<PyErr>> = Mutex::new(None);
        // The scope belongs to the calling pool, which is the decode pool. Fetching is then
        // installed onto its own pool, so `scope.spawn` from a fetch thread queues the decode
        // where it will not be blocked by whatever that fetch thread does next.
        rayon::scope(|scope| {
            fetch_pool(options.fetch_threads).install(|| {
                whole_units
                    .into_par_iter()
                    .for_each(|&(shard, group, wanted_by)| {
                        if failure.lock().unwrap().is_some() {
                            return;
                        }
                        let credit = Arc::new(budget.admit(group.length));
                        let (key, items) = &shards[shard];
                        let fetched = if group.length == 0 {
                            None // absent from the shard, so nothing to read
                        } else {
                        ...
                        for &(subchunk, offset, length) in &group.subchunks {
                            ...
                            scope.spawn(move |_| {
                                ...
                                // Dropping the credit here, and not before, is what makes the
                                // budget mean "bytes fetched but not yet decoded".
                                drop(credit);
                            });
                        }
                    });
