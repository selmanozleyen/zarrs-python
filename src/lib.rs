...
impl CodecPipelineImpl {
    ...
    /// no second Rust path.
    #[pyo3(signature = (chunk_items, value, io_workers=None, codec_workers=None, strict=false))]
    fn retrieve_chunk_items_and_apply_index(
        ...
        // Every width is a per-call decision, so none of them is a constructor argument. The one
        // width still not servable is above what a pool was built with, since it cannot grow.
        let config =
            read_decode::config_for_call(py, io_workers, codec_workers, strict, self.limits)?;
