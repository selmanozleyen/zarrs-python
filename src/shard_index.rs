...
impl ShardLayout {
    ...
    ///
    /// Two inner chunks share a read when the gap between them is at most `max_gap`. At the
    /// default of zero that means only chunks that touch, which costs nothing: not a byte outside
    /// the request is read. A larger `max_gap` reads unrequested bytes to save a request, a
    /// bandwidth-for-latency trade that depends on the storage, so it is the caller's to make.
    ///
    ...
    /// needing all of them collapses to a single read of the whole shard even at `max_gap` zero.
    ///
    /// A merged read never spans more than one shard, so the largest it can grow to is one shard.
    pub fn merge_reads(index: &[u64], mut subchunks: Vec<u64>, max_gap: u64) -> Vec<ReadGroup> {
        ...
        for subchunk in subchunks {
            ...
            match groups.last_mut() {
                // Close enough behind to extend the read rather than start another. Chunks laid
                // out of index order fail the `>=` and simply stay separate.
                Some(group)
                    if group.length > 0
                        && offset >= group.end()
                        && offset - group.end() <= max_gap =>
                {
                    group
                        .subchunks
