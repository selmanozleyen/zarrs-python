...
def _read(path: Path, values: np.ndarray) -> None:
    ...
    np.testing.assert_array_equal(zarr.open_array(path, mode="r")[selection], values[selection])
...
def test_a_ceiling_asked_for_after_the_pools_exist_warns(sharded_1d) -> None:
    ...
    with zarr.config.set(PIPELINE | {"codec_pipeline.read_worker_ceiling": absurd}):
