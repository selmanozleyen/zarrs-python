...
def test_asking_above_what_the_process_builds_warns(sharded_1d) -> None:
    ...
    with (
        zarr.config.set(PIPELINE | {"codec_pipeline.io_workers": absurd}),
        pytest.warns(UserWarning, match=rf"io_workers = {absurd} is above"),
    ):
        _read(path, values)
...
def test_strict_makes_it_an_error(sharded_1d) -> None:
    ...
    strict = PIPELINE | {
        "codec_pipeline.io_workers": absurd,
...
def test_a_width_below_the_maximum_is_honoured_after_the_pools_exist(sharded_1d) -> None:
    ...
    with zarr.config.set(PIPELINE | {"codec_pipeline.io_workers": 2}):
...
def test_the_width_actually_in_force_is_silent(sharded_1d) -> None:
    ...
    with zarr.config.set(PIPELINE | {"codec_pipeline.io_workers": built_read}):
        with warnings.catch_warnings(record=True) as record:
            warnings.simplefilter("always", UserWarning)
            _read(path, values)
        assert [str(w.message) for w in record] == []
