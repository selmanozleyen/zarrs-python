...
def test_decoder_cache_survives_a_write(tmp_path):
    """A cached decoder holds a shard index that a write invalidates.

    The cache is on by default, and it is only sound because
    `store_chunks_with_indices` evicts what it touches. This is the test that
    catches it if that ever stops happening -- stale reads here would be silent
    and correct-looking.
    """
