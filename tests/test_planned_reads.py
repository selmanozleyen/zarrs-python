...
@pytest.mark.parametrize("gap", [0, 1, 4096, 1 << 20, 1 << 40])
@pytest.mark.parametrize("selection", [*SELECTIONS, *INTEGER_SELECTIONS])
def test_merge_gap_changes_reads_not_results(
    sharded: tuple[Path, np.ndarray], selection: object, gap: int
) -> None:
    """`plan_reads_merge_gap_bytes` trades unrequested bytes for fewer requests. At 0 only touching
    reads merge; at a huge value everything in a shard becomes one read. Neither may move a
    byte of the answer."""
    path, expected = sharded
    with zarr.config.set(PLANNED | {"codec_pipeline.plan_reads_merge_gap_bytes": gap}):
...
def test_planning_is_skipped_when_the_array_is_not_sharded(tmp_path: Path) -> None:
    """`plan_reads` describes a shard layout, so an unsharded array must fall through to the
